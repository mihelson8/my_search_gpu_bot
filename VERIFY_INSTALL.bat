...
findstr /C:"2026.08.23-r25" "%STABLE%\anpr\version.py" >nul
if errorlevel 1 (
  echo [СТАРАЯ СБОРКА] Нужна 2026.08.23-r25
  ...
  echo [OK] В папке стоит сборка r25.
  echo Откройте программу через START_ANPR.bat из:
  echo   %STABLE%
  echo В окне должен быть ЖЁЛТЫЙ значок: СБОРКА 2026.08.23-r25
)
